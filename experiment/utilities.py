...
MODEL_TYPES = Literal['lr', 'cart', 'gbdt', 'nn', 'rf']
DATASET_TYPES = Literal['g', 'b', 'a']

def get_dataset_name(dataset: DATASET_TYPES):
    if dataset == 'g':
        dataset = 'Google'
    elif dataset == 'b':
        dataset = 'Backblaze'
...
def get_num_periods(dataset: DATASET_TYPES):
    if dataset == 'g':
        num_periods = 28
    elif dataset == 'b':
        num_periods = 36
...
def obtain_tuned_model(model_name: MODEL_TYPES, dataset: DATASET_TYPES, period: int, mode: Literal['w', 'p']):
    ...
    para_str = df[np.logical_and(np.logical_and(
        df['Dataset']==dataset, 
        df['Period']==period), 
        df['Model']==model_name)].iloc[0]['Hyper']
...
def obtain_param_dist(model_name: MODEL_TYPES):
    param_dist = None
...
def obtain_raw_model(model_name: MODEL_TYPES):
    if model_name == 'lr':
...
def obtain_intervals(dataset: DATASET_TYPES):
    '''
...
def obtain_period_data(dataset: DATASET_TYPES):
...
def obtain_data(dataset: DATASET_TYPES, interval='m'):
    if dataset == 'g':
...
def obtain_feature_names(dataset: DATASET_TYPES):
    if dataset == 'g':
    ...
    # TODO: add alibaba then optimize get_xxx_data functions 
